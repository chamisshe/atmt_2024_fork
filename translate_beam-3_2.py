...
from seq2seq.beam3_2 import BeamSearch, BeamSearchNode
...
def main(args):
    ...
    for i, sample in enumerate(progress_bar):
        ...
        print(f"len(searches)\t{len(searches)}")
        with torch.no_grad():
        ...
        # create a 0s tensor with the number of hypotheses.
        # each index corresponds to a hypothesis: 0=active, 1=completed.
        # used mainly for early stopping if all hypotheses are completed (all ones.)
        check_completed = torch.zeros(args.beam_size*batch_size)
        # Start generating further tokens until max sentence length reached
        for __ in range(args.max_len-1):
            # Get the current nodes to expand
            nodes = [n[1] for s in searches for n in s.get_current_beams()]
            # print(f"type(nodes[0])\t{type(nodes[0])}")
            # print(f"len(nodes)\t{len(nodes)}")
            if not (0 in check_completed):
                print("All beams ended in EOS. Exiting.")
            ...
            for i in range(log_probs.shape[0]):
                ...
                node = nodes[i]
                search = node.search
                # !SKIP IF BEAM MARKED AS COMPLETE
                if node.completed:
                    # add one item of padding
                    node.pad_one(tgt_dict.unk_idx)
                ...
                for j in range(args.beam_size):
                    ...
                    if next_word[-1] == tgt_dict.eos_idx:
                        ...
                        search.add(-node.eval(args.alpha), node)
                        # !MARK BEAM AS COMPLETED
                        # print("Hypothesis complete.")
                        node.completed = True
                        check_completed[i] = 1
