...
def get_args():
    ...
    # Add data arguments
    parser.add_argument('--data', default='indomain/raw_data/', help='path to data directory')
    ...
    parser.add_argument('--bpe-dropout', default=0, type=float, help='Set the probability for the BPE-Dropout. p=0 means no dropout.')
...
def main(args):
    ...
    src_file_raw=os.path.join(args.data, "..", "raw", '{:s}.{:s}'.format("train", args.source_lang))
    tgt_file_raw=os.path.join(args.data, "..", "raw", '{:s}.{:s}'.format("train", args.target_lang))
    joined_src_tgt = os.path.join(args.data, "..", "raw", "JOINED.txt")
    with open(joined_src_tgt, "w", encoding="utf8") as j,\
            open(src_file_raw, "r", encoding="utf8") as s,\
            open(tgt_file_raw, "r", encoding="utf8") as t:
        j.write(s.read()+t.read())


    # LEARN BPE MODEL
    bpe_path = "assignments/03/bpe"
    if not os.path.exists(bpe_path):
        os.mkdir(bpe_path)
    bpe_modelname = f'{bpe_path}/m'
    spm.SentencePieceTrainer.train(input=r"data\en-fr-bpe\raw\tiny_train.en", model_prefix=bpe_modelname, vocab_size=1000, model_type="bpe")

    
    # Load dictionaries
    # src_dict = Dictionary.load(os.path.join(args.data, 'dict.{:s}'.format(args.source_lang)))
    src_dict=f'{bpe_modelname}.vocab'
    logging.info('Loaded a source dictionary ({:s}) with {:d} words'.format(args.source_lang, len(src_dict)))
    # tgt_dict = Dictionary.load(os.path.join(args.data, 'dict.{:s}'.format(args.target_lang)))
    tgt_dict=f'{bpe_modelname}.vocab'
    logging.info('Loaded a target dictionary ({:s}) with {:d} words'.format(args.target_lang, len(tgt_dict)))
    
    #   join src and tgt files for joint vocab
